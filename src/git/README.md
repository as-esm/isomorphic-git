...
│   ├── heads/           # Local branches (future)
│   ├── tags/            # Tags (future)
│   ├── remotes/         # Remote-tracking branches (future)
│   └── packed-refs.ts   # Packed refs file (future)
...
As we migrate code to this structure:
- Old managers become thin wrappers
- Commands use `src/git/` functions directly
- Repository delegates to `src/git/` functions
- Eventually remove old managers/core-utils complexity
