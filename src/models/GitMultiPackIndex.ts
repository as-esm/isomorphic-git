...
export class GitMultiPackIndex {
   ...
   */
  static fromBuffer(midx: Buffer): GitMultiPackIndex {
    ...
    if (instance.objectIdVersion !== 1) {
      throw new InternalError(`Unsupported object ID version: ${instance.objectIdVersion} (only SHA-1 supported)`)
    }
    ...
    for (let i = 0; i < packfileCount; i++) {
      ...
      instance.packfileNames.push(Buffer.from(nameBytes).toString('ascii'))
    }
    ...
    for (let i = 0; i < totalObjects; i++) {
      const oidBytes = reader.slice(20)
    ...
    // Verify checksum
    const checksumOffset = midx.length - 20
    const data = midx.slice(0, checksumOffset)
    const expectedChecksum = midx.slice(checksumOffset).toString('hex')
    // Note: We'd need to compute SHA-1 here, but for now we'll skip verification
    // In production, you'd want to verify: const actualChecksum = await shasum(data)
