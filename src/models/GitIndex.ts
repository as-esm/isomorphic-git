...
export class GitIndex {
  ...
  static async fromBuffer(buffer: Buffer | Uint8Array): Promise<GitIndex> {
    ...
    while (!reader.eof() && i < numEntries) {
      ...
      const pathlength = buf.indexOf(0, reader.tell() + 1) - reader.tell()
      if (pathlength < 1) {
  ...
  *[Symbol.iterator](): Generator<IndexEntry, void, unknown> {
  ...
  async toObject(): Promise<Buffer> {
    ...
    const entryBuffers: Promise<Buffer>[] = []
    for (const entry of this.entries) {
    ...
    const resolvedBuffers = await Promise.all(entryBuffers)

    const body = Buffer.concat(resolvedBuffers)
