...
> {
  ...
  let lineOne: Buffer | null | true = await read()
  ...
  if (!Buffer.isBuffer(lineOne) || lineOne.toString('utf8').replace(/\n$/, '') !== `# service=${service}`) {
    throw new ParseError(`# service=${service}\\n`, Buffer.isBuffer(lineOne) ? lineOne.toString('utf8') : '')
  }
  let lineTwo: Buffer | null | true = await read()
  // skip past any flushes
  while (lineTwo === null) lineTwo = await read()
  // In the edge case of a brand new repo, zero refs (and zero capabilities)
  // are returned.
  if (lineTwo === true) return { protocolVersion: 1, capabilities, refs, symrefs }
  const lineTwoStr = Buffer.isBuffer(lineTwo) ? lineTwo.toString('utf8') : ''
