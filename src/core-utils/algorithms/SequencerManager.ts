 ...
 */
export const isRebaseInProgress = async ({ fs, gitdir }: { fs: FsClient; gitdir: string }): Promise<boolean> => {
 ...
 * Aborts a rebase
 */
export const abortRebase = async ({ fs, gitdir }: { fs: FsClient; gitdir: string }): Promise<void> => {
  const rebaseDir = getSequencerDir(gitdir, 'rebase')
  try {
 ...
 * Completes a rebase
 */
export const completeRebase = async ({ fs, gitdir }: { fs: FsClient; gitdir: string }): Promise<void> => {
