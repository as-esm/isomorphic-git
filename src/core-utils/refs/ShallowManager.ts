...
export class ShallowManager {
   ...
   */
  static async read({ fs, gitdir }: { fs: FsClient; gitdir: string }): Promise<Set<string>> {
   ...
   */
  static async write({ fs, gitdir, oids }: { fs: FsClient; gitdir: string; oids: Set<string> }): Promise<void> {
    const filepath = join(gitdir, 'shallow')
