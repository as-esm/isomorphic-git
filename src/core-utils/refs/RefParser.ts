 ...
 */
export const parseLooseRef = (buffer: Buffer | string): LooseRef => {
 ...
 */
export const serializePackedRefs = (refs: Map<string, string>): Buffer => {
  const lines: string[] = []
 ...
 */
export const serializeLooseRef = (ref: LooseRef): Buffer => {
  if (ref.symbolic && ref.target) {
    return Buffer.from(`ref: ${ref.target}\n`, 'utf8')
  }

  if (ref.oid) {
    return Buffer.from(`${ref.oid}\n`, 'utf8')
  }
