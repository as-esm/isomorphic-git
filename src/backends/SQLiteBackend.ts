...
export class SQLiteBackend implements GitBackend {
  ...
  async readHEAD(): Promise<string> {
    ...
    if (row) {
      return Buffer.from(row.value as Uint8Array).toString('utf8').trim()
    }
  ...
  async readDescription(): Promise<string | null> {
    ...
    if (row) {
      return Buffer.from(row.value as Uint8Array).toString('utf8')
    }
    return null
  }
  ...
  async writeRef(ref: string, value: string): Promise<void> {
    const db = await this._getDB()
    db.prepare('INSERT OR REPLACE INTO refs (ref, value) VALUES (?, ?)').run(ref, value + '\n')
  }
  ...
  async listRefs(prefix: string): Promise<string[]> {
    const db = await this._getDB()
    const rows = db.prepare('SELECT ref FROM refs WHERE ref LIKE ?').all(prefix + '%')
    return rows.map((row: { ref: string }) => row.ref)
  }

  async hasRef(ref: string): Promise<boolean> {
    const db = await this._getDB()
    const row = db.prepare('SELECT ref FROM refs WHERE ref = ?').get(ref)
    return !!row
  }

  async readPackedRefs(): Promise<string | null> {
    const db = await this._getDB()
    const row = db.prepare('SELECT data FROM packed_refs WHERE id = 1').get()
    return row ? (row.data as string) : null
  }
