...
export async function writeObject({
  ...
  try {
    ...
    let processedObject: Uint8Array = object as Uint8Array
    let processedFormat: 'deflated' | 'wrapped' | 'content' = format as 'deflated' | 'wrapped' | 'content'
    // Convert object to buffer
    if (format === 'parsed') {
      switch (type) {
        case 'commit':
          processedObject = GitCommit.from(object as CommitObject).toObject()
          break
        case 'tree':
          processedObject = GitTree.from(object as TreeObject).toObject()
          break
        case 'blob':
          processedObject = Buffer.from(object as string, encoding)
          break
        case 'tag':
          processedObject = GitAnnotatedTag.from(object as TagObject).toObject()
          break
        default:
    ...
    ;(err as { caller?: string }).caller = 'git.writeObject'
    throw err
  }
}
