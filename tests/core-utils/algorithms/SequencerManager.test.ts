...
import {
  ...
  abortRebase,
  completeRebase,
} from '../../../src/core-utils/algorithms/SequencerManager.ts'
import { makeFixture } from '../../helpers/fixture.ts'
import { init } from 'isomorphic-git'

test('SequencerManager', async (t) => {
  await t.test('getSequencerDir - rebase operation', () => {
  ...
  await t.test('getSequencerDir - cherry-pick operation', () => {
    const gitdir = '/test/.git'
  ...
  await t.test('getSequencerDir - default operation', () => {
    const gitdir = '/test/.git'
  ...
  await t.test('isRebaseInProgress - no rebase', async () => {
  ...
  await t.test('isRebaseInProgress - rebase in progress', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('readRebaseTodo - file does not exist', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('readRebaseTodo - read commands', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
...
`
  ...
  await t.test('readRebaseTodo - filters comments and empty lines', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
...
`
  ...
  await t.test('writeRebaseTodo - write commands', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('readRebaseHead - file does not exist', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('readRebaseHead - read head name', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('readRebaseOnto - file does not exist', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('readRebaseOnto - read onto OID', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('initRebase - initialize rebase', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('nextRebaseCommand - get next command', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('nextRebaseCommand - empty todo list', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('abortRebase - remove rebase directory', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('abortRebase - directory does not exist', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
  ...
  await t.test('completeRebase - remove rebase directory', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir: gitdir.replace('/.git', '') })
