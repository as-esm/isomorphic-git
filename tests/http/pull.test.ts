...
test('pull', async (t) => {
  ...
  await t.test('pull fast-forward only', async () => {
    ...
    // Ensure repository is not bare (needed for add command)
    // For bare repos, dir might be the same as gitdir, so we need to ensure
    // the working directory exists and is separate from gitdir
    const workDir = dir && dir !== gitdir ? dir : join(gitdir, '..', 'workdir')
    ...
    await fs.write(join(workDir, 'z.txt'), 'Hi')
    await add({ fs, dir: workDir, gitdir, filepath: 'z.txt' })
    await commit({ fs, dir: workDir, gitdir, message: 'Added z.txt', author })

    const logs = await log({ fs, gitdir, dir: workDir, ref: 'refs/heads/master' })
