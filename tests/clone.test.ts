...
import { join as joinPath } from '../src/utils/join.ts'
...
test('clone', async (t) => {
  ...
  await t.test('clone from local git repository', async () => {
    ...
    const sourceGitDir = joinPath(sourceDir, '.git')
    ...
    await sourceFs.write(joinPath(sourceDir, 'README.md'), '# Test Repository\n')
    ...
    await sourceFs.write(joinPath(sourceDir, 'file1.txt'), 'Content 1\n')
    execSync('git add file1.txt', { cwd: sourceDir })
    execSync('git commit -m "Add file1"', { cwd: sourceDir })
    
    await sourceFs.write(joinPath(sourceDir, 'file2.txt'), 'Content 2\n')
    ...
    // Verify repository was cloned
    assert.strictEqual(await fs.exists(dir), true)
    assert.strictEqual(await fs.exists(joinPath(gitdir, 'objects')), true)
    assert.strictEqual(await fs.exists(joinPath(gitdir, 'config')), true)
    
    // Verify remote was added
    const configService = new (await import('../src/utils/configAccess.ts')).ConfigAccess(fs, gitdir)
    ...
    assert.strictEqual(
      await fs.exists(joinPath(gitdir, 'refs/heads/feature-branch')),
      true
    )
    ...
    assert.strictEqual(await fs.exists(joinPath(dir, 'README.md')), true)
    assert.strictEqual(await fs.exists(joinPath(dir, 'file1.txt')), true)
    assert.strictEqual(await fs.exists(joinPath(dir, 'file2.txt')), true)
    
    // Verify file contents
    const readme = await fs.read(joinPath(dir, 'README.md'), 'utf8')
    assert.strictEqual(readme, '# Test Repository\n')
    
    const file1 = await fs.read(joinPath(dir, 'file1.txt'), 'utf8')
    assert.strictEqual(file1, 'Content 1\n')
    
    const file2 = await fs.read(joinPath(dir, 'file2.txt'), 'utf8')
