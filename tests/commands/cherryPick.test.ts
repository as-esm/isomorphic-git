...
import { cherryPick, init, commit, add, writeRef, resolveRef } from 'isomorphic-git'
...
test('cherryPick', async (t) => {
  ...
  await t.test('throws MissingParameterError when gitdir is missing and dir is not provided', async () => {
    ...
    try {
      ...
      // If it doesn't throw, that's because gitdir is derived from dir (which defaults to "")
      // This will fail later when trying to resolve the commit, which is acceptable
      // The test is checking that we handle missing parameters gracefully
    } catch (error) {
      // Could be MissingParameterError or NotFoundError (when trying to resolve commit)
      assert.ok(error instanceof Error)
      // If it's a MissingParameterError, it should be for gitdir
  ...
  await t.test('throws MissingParameterError when commit is missing', async () => {
    const { fs, gitdir } = await makeFixture('test-empty')
    try {
      await cherryPick({
        fs,
        gitdir,
        // @ts-expect-error - intentionally missing commit
      } as any)
      assert.fail('Should have thrown MissingParameterError')
    } catch (error) {
      assert.ok(error instanceof MissingParameterError)
  ...
  await t.test('throws error when commit does not exist', async () => {
  ...
  await t.test('throws error when commit is not a commit object', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
  ...
  await t.test('throws error when cherry-picking root commit (no parent)', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
  ...
  await t.test('throws error when HEAD is not a commit', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
  ...
  await t.test('successfully cherry-picks a commit with no conflicts', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
  ...
  await t.test('handles noCommit parameter - returns tree OID without committing', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    ...
    const { readSymbolicRef } = await import('../../src/git/refs/readRef.ts')
  ...
  await t.test('handles conflicts - throws error and sets CHERRY_PICK_HEAD', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    ...
    try {
      ...
      if ((error as Error).message.includes('conflict')) {
        ...
        const { StateManager } = await import('../../src/core-utils/StateManager.ts')
