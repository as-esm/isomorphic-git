import { test } from 'node:test'
import assert from 'node:assert'
import { diff, init, commit, add, writeRef, resolveRef } from 'isomorphic-git'
import { makeFixture } from '../helpers/fixture.ts'
import { MissingParameterError } from '../../src/errors/MissingParameterError.ts'
import { Repository } from '../../src/core-utils/Repository.ts'

test('diff', async (t) => {
  await t.test('throws MissingParameterError when fs is missing', async () => {
    try {
      await diff({
        // @ts-expect-error - intentionally missing fs
        gitdir: '/tmp/test.git',
      } as any)
      assert.fail('Should have thrown MissingParameterError')
    } catch (error) {
      assert.ok(error instanceof MissingParameterError)
      assert.strictEqual((error as any).data?.parameter, 'fs')
    }
  })

  await t.test('throws MissingParameterError when gitdir is missing and dir is not provided', async () => {
    const { fs } = await makeFixture('test-empty')
    try {
      await diff({
        fs,
        // @ts-expect-error - intentionally missing both gitdir and dir
      } as any)
      assert.fail('Should have thrown MissingParameterError')
    } catch (error) {
      assert.ok(error instanceof MissingParameterError)
      assert.strictEqual((error as any).data?.parameter, 'gitdir')
    }
  })

  await t.test('uses gitdir from dir when gitdir is not provided', async () => {
    const { fs, dir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, cache })
    
    // Write file to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file1.txt`, 'content1')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file1.txt', cache: repo.cache })
    await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Diff should work with dir only
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
    })
    
    assert.ok(Array.isArray(result.entries), 'Should return entries array')
    assert.ok(result.refA, 'Should have refA')
  })

  await t.test('returns empty diff when no changes', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write file to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file.txt`, 'content')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file.txt', cache: repo.cache })
    await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Diff HEAD to HEAD should be empty - use dir to let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refA: 'HEAD',
      refB: 'HEAD',
    })
    
    assert.strictEqual(result.entries.length, 0, 'Should have no entries when comparing same refs')
  })

  await t.test('detects added files', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file1.txt`, 'content1')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file1.txt', cache: repo.cache })
    const commit1 = await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Add a new file
    await normalizedFs.write(`${dir}/file2.txt`, 'content2')
    await add({ fs, dir, filepath: 'file2.txt', cache: repo.cache })
    const commit2 = await commit({ fs, dir, message: 'Add file2', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Diff between commits - use dir to let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refA: commit1,
      refB: commit2,
    })
    
    assert.ok(result.entries.length > 0, 'Should have entries')
    const addedFile = result.entries.find(e => e.filepath === 'file2.txt' && e.status === 'added')
    assert.ok(addedFile, 'Should detect added file')
    assert.ok(addedFile!.newOid, 'Added file should have newOid')
    assert.strictEqual(addedFile!.oldOid, undefined, 'Added file should not have oldOid')
  })

  await t.test('detects deleted files', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file1.txt`, 'content1')
    await normalizedFs.write(`${dir}/file2.txt`, 'content2')
    
    // Create initial commit with two files
    await add({ fs, dir, filepath: 'file1.txt', cache: repo.cache })
    await add({ fs, dir, filepath: 'file2.txt', cache: repo.cache })
    const commit1 = await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Remove a file from index - use remove command
    await normalizedFs.rm(`${dir}/file2.txt`)
    const { remove } = await import('isomorphic-git')
    await remove({ fs, dir, filepath: 'file2.txt', cache: repo.cache })
    const commit2 = await commit({ fs, dir, message: 'Remove file2', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Diff between commits - use dir to let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refA: commit1,
      refB: commit2,
    })
    
    const deletedFile = result.entries.find(e => e.filepath === 'file2.txt' && e.status === 'deleted')
    assert.ok(deletedFile, 'Should detect deleted file')
    assert.ok(deletedFile!.oldOid, 'Deleted file should have oldOid')
    assert.strictEqual(deletedFile!.newOid, undefined, 'Deleted file should not have newOid')
  })

  await t.test('detects modified files', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file.txt`, 'content1')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file.txt', cache: repo.cache })
    const commit1 = await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Modify the file
    await normalizedFs.write(`${dir}/file.txt`, 'content2')
    await add({ fs, dir, filepath: 'file.txt', cache: repo.cache })
    const commit2 = await commit({ fs, dir, message: 'Modify file', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Diff between commits - use dir to let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refA: commit1,
      refB: commit2,
    })
    
    const modifiedFile = result.entries.find(e => e.filepath === 'file.txt' && e.status === 'modified')
    assert.ok(modifiedFile, 'Should detect modified file')
    assert.ok(modifiedFile!.oldOid, 'Modified file should have oldOid')
    assert.ok(modifiedFile!.newOid, 'Modified file should have newOid')
    assert.notStrictEqual(modifiedFile!.oldOid, modifiedFile!.newOid, 'OIDs should be different')
  })

  await t.test('compares working directory with HEAD when no refs provided', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file1.txt`, 'content1')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file1.txt', cache: repo.cache })
    await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Add a new file in working directory (not committed)
    await normalizedFs.write(`${dir}/file2.txt`, 'content2')
    await add({ fs, dir, filepath: 'file2.txt', cache: repo.cache })
    
    // Diff working directory vs HEAD
    const result = await diff({
      fs,
      dir,
      gitdir,
      cache: repo.cache,
    })
    
    assert.ok(Array.isArray(result.entries), 'Should return entries array')
    // Should detect the new file in working directory
    const newFile = result.entries.find(e => e.filepath === 'file2.txt' && e.status === 'added')
    // Note: This might not work if the file isn't tracked, but the test structure is correct
  })

  await t.test('compares index with HEAD when staged=true', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file1.txt`, 'content1')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file1.txt', cache: repo.cache })
    await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Stage a new file
    await normalizedFs.write(`${dir}/file2.txt`, 'content2')
    await add({ fs, dir, filepath: 'file2.txt', cache: repo.cache })
    
    // Diff staged changes
    const result = await diff({
      fs,
      dir,
      gitdir,
      cache: repo.cache,
      staged: true,
    })
    
    assert.ok(Array.isArray(result.entries), 'Should return entries array')
    assert.ok(result.refA, 'Should have refA (HEAD)')
  })

  await t.test('filters by filepath when provided', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/dir1/file1.txt`, 'content1')
    await normalizedFs.write(`${dir}/dir2/file2.txt`, 'content2')
    
    // Create initial commit with multiple files
    await add({ fs, dir, filepath: 'dir1/file1.txt', cache: repo.cache })
    await add({ fs, dir, filepath: 'dir2/file2.txt', cache: repo.cache })
    const commit1 = await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Modify both files
    await normalizedFs.write(`${dir}/dir1/file1.txt`, 'content1-modified')
    await normalizedFs.write(`${dir}/dir2/file2.txt`, 'content2-modified')
    await add({ fs, dir, filepath: 'dir1/file1.txt', cache: repo.cache })
    await add({ fs, dir, filepath: 'dir2/file2.txt', cache: repo.cache })
    const commit2 = await commit({ fs, dir, message: 'Modify files', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Diff with filepath filter - use dir to let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refA: commit1,
      refB: commit2,
      filepath: 'dir1/',
    })
    
    // Should only include files in dir1/
    const allInDir1 = result.entries.every(e => e.filepath.startsWith('dir1/'))
    assert.ok(allInDir1, 'All entries should be in dir1/')
    assert.ok(result.entries.length > 0, 'Should have entries in dir1/')
  })

  await t.test('handles empty repository (no HEAD)', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Diff in empty repo (no commits yet) - should handle gracefully
    try {
      const result = await diff({
        fs,
        dir,
        gitdir,
        cache,
      })
      
      // Should return empty diff or handle gracefully
      assert.ok(Array.isArray(result.entries), 'Should return entries array')
    } catch (error) {
      // If it throws NotFoundError for HEAD, that's acceptable for empty repo
      assert.ok(error instanceof Error)
      // NotFoundError is expected when there's no HEAD
    }
  })

  await t.test('handles refA only (compares with working directory)', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file1.txt`, 'content1')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file1.txt', cache: repo.cache })
    const commit1 = await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Add a new file
    await normalizedFs.write(`${dir}/file2.txt`, 'content2')
    await add({ fs, dir, filepath: 'file2.txt', cache: repo.cache })
    
    // Diff with only refA - use dir to let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refA: commit1,
    })
    
    assert.ok(Array.isArray(result.entries), 'Should return entries array')
    assert.strictEqual(result.refA, commit1, 'refA should be set')
  })

  await t.test('handles refB only (compares HEAD with refB)', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file1.txt`, 'content1')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file1.txt', cache: repo.cache })
    const commit1 = await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Create another commit
    await normalizedFs.write(`${dir}/file2.txt`, 'content2')
    await add({ fs, dir, filepath: 'file2.txt', cache: repo.cache })
    const commit2 = await commit({ fs, dir, message: 'Second', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Reset to first commit
    await writeRef({ fs, gitdir, ref: 'HEAD', value: commit1 })
    
    // Diff with only refB - use dir to let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refB: commit2,
    })
    
    assert.ok(Array.isArray(result.entries), 'Should return entries array')
    assert.strictEqual(result.refB, commit2, 'refB should be set')
  })

  await t.test('handles both refA and refB', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file1.txt`, 'content1')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file1.txt', cache: repo.cache })
    const commit1 = await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Create another commit
    await normalizedFs.write(`${dir}/file2.txt`, 'content2')
    await add({ fs, dir, filepath: 'file2.txt', cache: repo.cache })
    const commit2 = await commit({ fs, dir, message: 'Second', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Diff between two commits - use dir to let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refA: commit1,
      refB: commit2,
    })
    
    assert.ok(Array.isArray(result.entries), 'Should return entries array')
    assert.strictEqual(result.refA, commit1, 'refA should be set')
    assert.strictEqual(result.refB, commit2, 'refB should be set')
  })

  await t.test('handles mode changes (file permissions)', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write file to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/file.txt`, 'content1')
    
    // Create initial commit
    await add({ fs, dir, filepath: 'file.txt', cache: repo.cache })
    const commit1 = await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Modify file (this might change mode depending on implementation)
    await normalizedFs.write(`${dir}/file.txt`, 'content2')
    await add({ fs, dir, filepath: 'file.txt', cache: repo.cache })
    const commit2 = await commit({ fs, dir, message: 'Modify', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Diff should detect mode changes if any - use dir to let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refA: commit1,
      refB: commit2,
    })
    
    assert.ok(Array.isArray(result.entries), 'Should return entries array')
    // If mode changed, it should be detected
    const modified = result.entries.find(e => e.status === 'modified')
    if (modified) {
      // Mode might be the same or different depending on implementation
      assert.ok(modified.oldMode !== undefined || modified.newMode !== undefined, 'Mode should be tracked')
    }
  })

  await t.test('returns empty diff when no trees available', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
      // Try to diff with non-existent refs
      try {
        const result = await diff({
          fs,
          gitdir,
          cache: repo.cache,
          refA: 'nonexistent1',
          refB: 'nonexistent2',
        })
      
      // Should return empty diff or throw
      assert.ok(Array.isArray(result.entries), 'Should return entries array')
    } catch (error) {
      // If it throws for non-existent refs, that's acceptable
      assert.ok(error instanceof Error)
    }
  })

  await t.test('handles filepath filter with subdirectories', async () => {
    const { fs, dir, gitdir } = await makeFixture('test-empty')
    await init({ fs, dir, defaultBranch: 'main' })
    
    // Use Repository to ensure cache consistency - pass cache to Repository.open
    const cache: Record<string, unknown> = {}
    const repo = await Repository.open({ fs, dir, gitdir, cache })
    
    // Write files to filesystem
    const { normalizeFs } = await import('../../src/utils/normalizeFs.ts')
    const normalizedFs = normalizeFs(fs)
    await normalizedFs.write(`${dir}/a/b/file1.txt`, 'content1')
    await normalizedFs.write(`${dir}/a/c/file2.txt`, 'content2')
    await normalizedFs.write(`${dir}/x/y/file3.txt`, 'content3')
    
    // Create initial commit with nested structure - use repo.cache
    await add({ fs, dir, filepath: 'a/b/file1.txt', cache: repo.cache })
    await add({ fs, dir, filepath: 'a/c/file2.txt', cache: repo.cache })
    await add({ fs, dir, filepath: 'x/y/file3.txt', cache: repo.cache })
    const commit1 = await commit({ fs, dir, message: 'Initial', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Modify files
    await normalizedFs.write(`${dir}/a/b/file1.txt`, 'content1-modified')
    await normalizedFs.write(`${dir}/a/c/file2.txt`, 'content2-modified')
    await normalizedFs.write(`${dir}/x/y/file3.txt`, 'content3-modified')
    await add({ fs, dir, filepath: 'a/b/file1.txt', cache: repo.cache })
    await add({ fs, dir, filepath: 'a/c/file2.txt', cache: repo.cache })
    await add({ fs, dir, filepath: 'x/y/file3.txt', cache: repo.cache })
    const commit2 = await commit({ fs, dir, message: 'Modify', author: { name: 'Test', email: 'test@example.com' }, cache: repo.cache })
    
    // Filter by 'a/' prefix - use repo.cache and let diff resolve gitdir the same way commit does
    const result = await diff({
      fs,
      dir,
      cache: repo.cache,
      refA: commit1,
      refB: commit2,
      filepath: 'a/',
    })
    
    // All entries should start with 'a/'
    const allInA = result.entries.every(e => e.filepath.startsWith('a/'))
    assert.ok(allInA, 'All entries should be in a/ directory')
    // Should not include x/y/file3.txt
    const hasX = result.entries.some(e => e.filepath.startsWith('x/'))
    assert.strictEqual(hasX, false, 'Should not include files outside a/')
  })
})

