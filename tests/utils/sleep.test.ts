...
test('sleep', async (t) => {
  await t.test('resolves after specified milliseconds', async () => {
  ...
  await t.test('handles zero delay', async () => {
    const start = Date.now()
    await sleep(0)
    const elapsed = Date.now() - start
  ...
  await t.test('handles small delays', async () => {
    const start = Date.now()
    await sleep(10)
    const elapsed = Date.now() - start
  ...
  await t.test('can be chained', async () => {
    const start = Date.now()
    await sleep(20)
    await sleep(20)
    const elapsed = Date.now() - start
