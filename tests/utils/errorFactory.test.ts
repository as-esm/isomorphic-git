...
import { createErrorClass, createTypedErrorClass } from '../../src/utils/errorFactory.ts'
import { BaseError } from '../../src/errors/BaseError.ts'

test('errorFactory', async (t) => {
  await t.test('createErrorClass - with string message', () => {
  ...
  await t.test('createErrorClass - with function message', () => {
  ...
  await t.test('createErrorClass - with custom message', () => {
  ...
  await t.test('createErrorClass - with data', () => {
  ...
  await t.test('createErrorClass - static code property', () => {
    const TestError = createErrorClass('MY_ERROR', 'Message')

    assert.strictEqual(TestError.code, 'MY_ERROR')
  })

  await t.test('createTypedErrorClass - with message builder', () => {
  ...
  await t.test('createTypedErrorClass - static code property', () => {
    const TestError = createTypedErrorClass('TYPED_ERROR', () => 'Message')

    assert.strictEqual(TestError.code, 'TYPED_ERROR')
  })

  await t.test('createTypedErrorClass - complex data', () => {
  ...
  await t.test('createErrorClass - error inheritance', () => {
