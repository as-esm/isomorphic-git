...
import { formatInfoRefs } from '../../src/utils/formatInfoRefs.ts'

test('formatInfoRefs', async (t) => {
  await t.test('formats refs without prefix', () => {
    const remote = {
      refs: new Map([
  ...
  await t.test('filters refs by prefix', () => {
    const remote = {
      refs: new Map([
        ['refs/heads/main', 'abc123'],
  ...
  await t.test('includes symrefs when symrefs is true', () => {
    const remote = {
      refs: new Map([
        ['refs/heads/main', 'abc123'],
      ]),
  ...
  await t.test('excludes symrefs when symrefs is false', () => {
    const remote = {
      refs: new Map([
        ['refs/heads/main', 'abc123'],
      ]),
  ...
  await t.test('peels tags when peelTags is true', () => {
    const remote = {
      refs: new Map([
  ...
  await t.test('skips peeled tags when peelTags is false', () => {
    const remote = {
      refs: new Map([
        ['refs/tags/v1.0.0', 'abc123'],
        ['refs/tags/v1.0.0^{}', 'def456'], // Peeled tag
  ...
  await t.test('handles peeled tag not immediately after original tag', () => {
    const remote = {
      refs: new Map([
        ['refs/tags/v1.0.0', 'abc123'],
  ...
  await t.test('throws error when peeled tag has no matching original tag', () => {
    const remote = {
      refs: new Map([
  ...
  await t.test('handles empty refs map', () => {
    const remote = {
      refs: new Map(),
      symrefs: new Map(),
    }
  ...
  await t.test('handles multiple peeled tags for different tags', () => {
    const remote = {
      refs: new Map([
        ['refs/tags/v1.0.0', 'abc123'],
        ['refs/tags/v1.0.0^{}', 'def456'],
