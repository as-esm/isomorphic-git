...
import { normalizeStats } from '../../src/utils/normalizeStats.ts'

test('normalizeStats', async (t) => {
  await t.test('normalizes stats with seconds and nanoseconds', () => {
    const input = {
  ...
  await t.test('normalizes stats with milliseconds', () => {
    const input = {
  ...
  await t.test('normalizes stats with Date objects', () => {
  ...
  await t.test('uses current time when no time specified', () => {
  ...
  await t.test('handles negative size by setting to 0', () => {
    const input = {
      size: -1,
      mode: 0o100644,
    }
  ...
  await t.test('handles values exceeding MAX_UINT32', () => {
    const MAX_UINT32 = 2 ** 32
    const input = {
  ...
  await t.test('prioritizes seconds/nanoseconds over milliseconds', () => {
    const input = {
      ctimeSeconds: 1234567890,
      ctimeNanoseconds: 123456789,
  ...
  await t.test('prioritizes milliseconds over Date', () => {
  ...
  await t.test('normalizes mode using normalizeMode', () => {
    const input = {
      mode: 0o100644,
    }
    const result = normalizeStats(input)
