...
import { toHex } from '../../src/utils/toHex.ts'

test('toHex', async (t) => {
  await t.test('converts Buffer to hex string', () => {
    const buffer = Buffer.from([0x00, 0xFF, 0x0A, 0x1F])
    const result = toHex(buffer)
    assert.strictEqual(result, '00ff0a1f')
  })

  await t.test('pads single digit bytes with leading zero', () => {
  ...
  await t.test('handles all byte values', () => {
