...
import { normalizeIdentity } from '../../src/utils/normalizeIdentity.ts'
...
test('normalizeIdentity', async (t) => {
  await t.test('normalize identity - author type with provided values (bypasses config)', async () => {
  ...
  await t.test('normalize identity - committer type with provided values', async () => {
    const { fs, gitdir, dir } = await makeFixture('test-empty')
    await init({ fs, dir })
  ...
  await t.test('normalize identity - author priority: provided > commit > default', async () => {
    const { fs, gitdir, dir } = await makeFixture('test-empty')
    await init({ fs, dir })
  ...
  await t.test('normalize identity - committer priority: provided > fallback > commit > default', async () => {
    const { fs, gitdir, dir } = await makeFixture('test-empty')
    await init({ fs, dir })
    await setConfig({ fs, dir, path: 'user.name', value: 'Config Name' })
    await setConfig({ fs, dir, path: 'user.email', value: 'config@example.com' })
  ...
  await t.test('normalize identity - returns undefined if no name', async () => {
    const { fs, gitdir, dir } = await makeFixture('test-empty')
    await init({ fs, dir })
  ...
  await t.test('normalize identity - uses fallback for committer when commit.committer missing', async () => {
    const { fs, gitdir, dir } = await makeFixture('test-empty')
    await init({ fs, dir })
    await setConfig({ fs, dir, path: 'user.name', value: 'Config Name' })
    await setConfig({ fs, dir, path: 'user.email', value: 'config@example.com' })
  ...
  await t.test('normalize identity - uses commit.committer for committer type', async () => {
    const { fs, gitdir, dir } = await makeFixture('test-empty')
    await init({ fs, dir })
    await setConfig({ fs, dir, path: 'user.name', value: 'Config Name' })
