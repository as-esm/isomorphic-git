...
import { shasum } from '../../src/utils/shasum.ts'

test('shasum', async (t) => {
  await t.test('calculates SHA-1 hash for Buffer', async () => {
  ...
  await t.test('calculates SHA-1 hash for Uint8Array', async () => {
    const input = new Uint8Array(Buffer.from('test'))
    const result = await shasum(input)
    assert.strictEqual(result, 'a94a8fe5ccb19ba61c4c0873d391e987982fbbd3')
  })

  await t.test('calculates SHA-1 hash for empty buffer', async () => {
  ...
  await t.test('calculates SHA-1 hash for longer string', async () => {
  ...
  await t.test('calculates SHA-1 hash for binary data', async () => {
  ...
  await t.test('produces consistent results for same input', async () => {
  ...
  await t.test('produces different results for different inputs', async () => {
  ...
  await t.test('handles unicode strings', async () => {
