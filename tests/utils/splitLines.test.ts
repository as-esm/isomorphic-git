...
import { splitLines } from '../../src/utils/splitLines.ts'

test('splitLines', async (t) => {
  await t.test('splits lines with newline', async () => {
  ...
  await t.test('splits lines with carriage return', async () => {
  ...
  await t.test('splits lines with CRLF', async () => {
    const input = [Buffer.from('line1\r\nline2\r\nline3')]
    const output = splitLines(input)
  ...
  await t.test('handles multiple chunks', async () => {
    const input = [
      Buffer.from('line1\nline'),
      Buffer.from('2\nline3\n'),
    ]
  ...
  await t.test('handles empty input', async () => {
    const input: Buffer[] = []
    const output = splitLines(input)
  ...
  await t.test('handles input without line breaks', async () => {
  ...
  await t.test('handles Uint8Array input', async () => {
  ...
  await t.test('handles mixed line endings', async () => {
    const input = [Buffer.from('line1\nline2\rline3\r\nline4')]
    const output = splitLines(input)
