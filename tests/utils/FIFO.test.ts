...
import { FIFO } from '../../src/utils/FIFO.ts'

test('FIFO', async (t) => {
  await t.test('writes and reads buffers in order', async () => {
    const fifo = new FIFO()
  ...
  await t.test('handles async reading', async () => {
    const fifo = new FIFO()
  ...
  await t.test('ends iteration when end() is called', async () => {
    const fifo = new FIFO()
  ...
  await t.test('handles async end', async () => {
    const fifo = new FIFO()
    const readPromise = fifo.next()
  ...
  await t.test('throws error when writing after end', () => {
    const fifo = new FIFO()
    fifo.end()
  ...
  await t.test('destroys with error', () => {
    const fifo = new FIFO()
  ...
  await t.test('throws error on concurrent next() calls', async () => {
    const fifo = new FIFO()
  ...
  await t.test('handles empty queue with end', async () => {
    const fifo = new FIFO()
    fifo.end()
  ...
  await t.test('processes all queued items before ending', async () => {
    const fifo = new FIFO()
    fifo.write(Buffer.from([1]))
