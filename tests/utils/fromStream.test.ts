...
import { fromStream } from '../../src/utils/fromStream.ts'

test('fromStream', async (t) => {
  await t.test('convert ReadableStream to async iterator', async () => {
  ...
  await t.test('handle empty stream', async () => {
    const stream = new ReadableStream({
      start(controller) {
        controller.close()
      },
  ...
  await t.test('handle stream with single chunk', async () => {
    const stream = new ReadableStream({
      start(controller) {
  ...
  await t.test('handle stream that already has asyncIterator', async () => {
  ...
  await t.test('iterator is async iterable', async () => {
    const stream = new ReadableStream({
      start(controller) {
  ...
  await t.test('handle binary data', async () => {
