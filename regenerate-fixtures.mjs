...
const FileSystemModule = await import('./src/models/FileSystem.ts')
const TreeParserModule = await import('./src/core-utils/parsers/Tree.ts')
const GitObjectModule = await import('./src/models/GitObject.ts')
...
// Check for fixtures in new location first, fallback to old location
const newFixturesDir = path.join(__dirname, 'tests', '__fixtures__')
const oldFixturesDir = path.join(__dirname, '__tests__', '__fixtures__')
const fixturesDir = fs.existsSync(newFixturesDir) ? newFixturesDir : oldFixturesDir
  ...
  for (const subdir of subdirs) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // Inflate the object
        const { inflate } = await import('./src/core-utils/Zlib.ts')
        ...
        if (type === 'tree') {
          ...
          const { shasum } = await import('./src/core-utils/ShaHasher.ts')
