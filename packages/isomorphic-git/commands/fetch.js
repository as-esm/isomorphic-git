 ...
 * @see FetchResult
 */
export async function _fetch({ fs, cache, http, onProgress, onMessage, onAuth, onAuthSuccess, onAuthFailure, gitdir, ref: _ref, remoteRef: _remoteRef, remote: _remote, url: _url, corsProxy, depth = null, since = null, exclude = [], relative = false, tags = false, singleBranch = false, headers = {}, prune = false, pruneTags = false, }) {
    ...
    for (const oid of response.shallows) {
        if (!oids.has(oid)) {
            ...
            try {
                ...
                const hasParents = await Promise.all(commit
                    .headers()
