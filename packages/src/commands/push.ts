 ...
 * @param {import('typedefs-http.ts').HttpClient} args.http
 * @param {import('typedefs-http.ts').ProgressCallback} [args.onProgress]
 ...
 *
 * @returns {Promise<import('../typedefs.js').PushResult>}
 */
export async function _push({
  ...
  const packstream1 = await writeReceivePackRequest({
    capabilities,
    // @ts-expect-error not assign able to never
